...
from graph.models import Node, NodeType, EdgeType
import re
...
def _safe_filename(value: str) -> str:
    """Crée un segment de nom de fichier sûr sur tous les OS."""
...
def generate_module_dependency_diagram(
    ...
    """
    Diagramme de dépendances entre modules (imports uniquement).
    Limité aux noeuds internes (pas les dépendances externes).
    """
    ...
    # Arêtes IMPORTS uniquement entre modules internes
    for edge in edges:
        if (
...
def generate_class_diagram(
    ...
    """
    Diagramme classes/méthodes pour un module donné.
    file_filter : si fourni, limite au fichier spécifié.
    """
...
def generate_erd(nodes: dict[str, Node], edges: list) -> str:
    ...
    # Définition des tables avec colonnes
...
def export_all_diagrams(
    ...
    """Génère et écrit tous les diagrammes, retourne un dict nom -> contenu."""
